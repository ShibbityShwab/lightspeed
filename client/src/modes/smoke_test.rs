...
pub async fn run_smoke_test(proxy_addr: SocketAddrV4) -> anyhow::Result<()> {
    ...
    let echo_sock = tokio::net::UdpSocket::bind(
        SocketAddrV4::new(Ipv4Addr::new(127, 0, 0, 1), echo_port)).await?;
    ...
    // 2. Proxy
    let proxy_bin = std::env::current_exe().unwrap_or_default()
        .parent().map(|p| p.join("lightspeed-proxy")).unwrap_or_default();
    ...
    interceptor.check_availability().map_err(|e| anyhow::anyhow!("{}", e))?;
    let config = crate::interceptor::InterceptorConfig {
        game_name: "SmokeTest".into(), pid: None,
        port_range: (echo_port, echo_port + 1),
        initial_routes: vec![],
        proxy_addr, fec_enabled: false, fec_k: 4,
    };
    ...
    handle.stop();
    echo_handle.abort();
    if let Some(mut c) = proxy_child { let _ = c.kill(); let _ = c.wait(); }
